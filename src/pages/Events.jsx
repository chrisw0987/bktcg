          ...
          <div className="space-y-12">
            {displayedEvents.map((event) => {
              ...
              return (
                <div
                  key={event.id}
                  className="overflow-hidden rounded-3xl border-2 border-[#4FCBFF]/20 bg-[#F8F7FF] shadow-[0_15px_40px_rgba(0,0,0,.15)]"
                >
                  {/* EVENT POSTER */}
                  {event.image && (
                    <div className="border-b-2 border-[#4FCBFF]/20 bg-gradient-to-br from-[#0B1D5A] via-[#2A1E68] to-[#5B2DB8] p-5 md:p-8">
                      <div className="mx-auto flex max-w-4xl justify-center">
                        <img
                          src={event.image}
                          alt={`${event.title} event poster`}
                          className="max-h-[650px] w-auto max-w-full rounded-2xl object-contain shadow-2xl"
                        />
                      </div>
                    </div>
                  )}
